...
namespace Blaise2.Ast
{
    public partial class AstEvaluator
    {
        ...
        private bool ContainsNoDuplicateFunctionSignatures(IEnumerable<FunctionNode> functions)
        {
            var valid = true;
            var seen = new HashSet<BlaiseSignature>();
            foreach (var node in functions)
            {
                var sig = BlaiseSignatureFactory.MakeFunctionSignature(node);
                if (!seen.Add(sig))
                {
        ...
        private bool ContainsNoDuplicateVariables(List<VarDeclNode> varDecls)
        {
            var valid = true;
        ...
        private bool CallSignatureMatchesFunctionSignature(FunctionCallNode node)
        {
